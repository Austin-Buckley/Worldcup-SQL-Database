...
echo "$($PSQL "SELECT name FROM teams WHERE team_id = (SELECT winner_id FROM games WHERE year = 2018 GROUP BY winner_id HAVING COUNT(*) = (SELECT MAX(cnt) FROM (SELECT COUNT(*) AS cnt FROM games WHERE year = 2018 GROUP BY winner_id) AS max_counts))")"
...
echo "$($PSQL "SELECT DISTINCT ON (year) year || '|' || (SELECT name FROM teams WHERE team_id = winner_id) FROM games WHERE round = 'Final' ORDER BY year")"
